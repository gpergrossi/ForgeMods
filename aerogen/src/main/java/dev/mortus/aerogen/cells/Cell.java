...
public class Cell {		
	...
	public Cell(InfiniteVoronoi container, Point cellKey) {
		...
		this.site = new Vec2((cellX+0.5)*container.gridSize + Math.cos(a)*r, (cellY+0.5)*container.gridSize + Math.sin(a)*r);
	}
	 ...
	 * @return
	 */
	protected static Cell[] initRange(InfiniteVoronoi container, int minCellX, int minCellY, int maxCellX, int maxCellY, boolean reserve) {		
		int width = maxCellX - minCellX + 1;
		int height = maxCellY - minCellY + 1;
		Cell[] results = new Cell[width * height];
		...
		Cell[] workCells = new Cell[workWidth * workHeight];
		...
		for (int j = 0; j < workHeight; j++) {
			for (int i = 0; i < workWidth; i++) {
				Cell cell = container.peakCell(new Point(minCellX-padding+i, minCellY-padding+j));
		...
		for (int j = 0; j < height; j++) {
			for (int i = 0; i < width; i++) {
				Cell cell = workCells[(j+padding)*workWidth+(i+padding)];
				...
				if (!cell.initialized) {
					...
					for (Edge edge : site.getEdges()) {
						Vec2 point = edge.getNeighbor(site).point;
						Optional<Cell> neighbor = java.util.Arrays.stream(workCells).filter(n -> n.site == point).findFirst();
