...
public class WeightedList<T> {
	...
	public Tuple2<T, Integer> removeRandom(Random random) {
		WeightedTreeNode node = getRandomNode(random);
		if (node == null) return new Tuple2<>(null, 0);
		Tuple2<T, Integer> result = new Tuple2<>(node.item, node.weight);
		...
		private void remove() {			
			...
			if (leftChild == null) {
				...
				if (rightChild == null) {
					...
					AbstractBinaryNode.swap(this, toRemove);
					toRemove.remove();
					return;
				}
			}
